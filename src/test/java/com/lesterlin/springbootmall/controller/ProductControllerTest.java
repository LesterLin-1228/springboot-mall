...
public class ProductControllerTest {
    ...
    public void createProduct_success() throws Exception {
        ...
        // 建立一個 MockMvc 的 POST 請求的 RequestBuilder 物件
        RequestBuilder requestBuilder = MockMvcRequestBuilders
                .post("/products") // 發送 POST 請求至 "/products" 路徑
                .contentType(MediaType.APPLICATION_JSON) // 設定請求的內容類型為 JSON
                .content(json); // 設定請求的內容為 JSON 格式的資料 (存放在 json 變數中)

        mockMvc.perform(requestBuilder)
                // 印出請求和回應的詳細資訊到控制台
                .andDo(print())
                // 預期回應的 HTTP 狀態碼是 201 (Created)
                .andExpect(status().is(201))
                // 驗證 JSON 回應的特定屬性值是否符合預期
